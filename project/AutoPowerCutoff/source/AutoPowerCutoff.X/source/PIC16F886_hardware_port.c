...
#define SW_START_PORT			(PORTA)
#define SW_START_BIT			(0x04)
#define SW_CANCEL_PORT			(PORTA)
#define SW_CANCEL_BIT			(0x08)
...
void HW_PORT_Update( void )
{
	...
	if( REG_READ_08( SW_START_PORT ) & SW_START_BIT ){
		if( g_u08PortActiveCount_Ary[ eINPUT_PORT_START ] < 0xFF ){
			g_u08PortActiveCount_Ary[ eINPUT_PORT_START ]++;
		}
	}else{
		g_u08PortActiveCount_Ary[ eINPUT_PORT_START ] = 0;
	}

	if( REG_READ_08( SW_CANCEL_PORT ) & SW_CANCEL_BIT ){
		if( g_u08PortActiveCount_Ary[ eINPUT_PORT_CANCEL ] < 0xFF ){
			g_u08PortActiveCount_Ary[ eINPUT_PORT_CANCEL ]++;
		}
	}else{
		g_u08PortActiveCount_Ary[ eINPUT_PORT_CANCEL ] = 0;
	}
}
